            ...
            <nav role="navigation" className="relative flex items-center justify-around w-full">
                {buttons.map((button, index) => (
                    <button
                        ...
                        className={`${expanded ? 'h-10 w-10 lg:w-8 lg:h-8' : 'h-1.5 w-1.5'} rounded-full flex items-center justify-center bg-amber-50 border-[1px] border-amber-900 transition-all z-10 hover:border-[2px]`}
                    ...
                    >
                        {expanded && <span className="text-sm font-header text-amber-900">{button.label}</span>}
                    </button>
                ))}
            </nav>
        </div>
